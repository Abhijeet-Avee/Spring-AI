...
import com.spring.ollama.chatbot_rag_ollama.service.AdvisorChatService;
import com.spring.ollama.chatbot_rag_ollama.tools.DateTimeTool;
...
public class AdvisorChatServiceImpl implements AdvisorChatService {
	...
	public String advisorChat(String query) {
		...
		var chatResponse2 = this.chatClient.prompt().advisors(new SimpleLoggerAdvisor()) // advisor to log the messages
	...
	public Flux<String> advisorStreamChat(String query) {
		...
		var streamResponse = this.chatClient.prompt().system(this.systemMessageResource) // system message from file
				.user(user -> user.text(this.userMessageResource) // user message from file
						.param("concept", query))
				.stream().content();
	...
	@Override
	public String dateTimeToolChat(String query) {

		// Using DateTimeTool with advisor in SPRING AI	
		var chatResponse2 = this.chatClient.prompt()
				.tools(new DateTimeTool()) // adding date time tool
